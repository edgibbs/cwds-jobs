...
public interface JobConfiguration {
  ...
  static <T extends JobConfiguration> T getJobsConfiguration(Class<T> clazz, String path) {
    ...
    ConfigurationSourceProvider configurationSourceProvider = new SubstitutingSourceProvider(
        new FileConfigurationSourceProvider(), environmentVariableSubstitutor);
    try {
      T configuration = new YamlConfigurationFactory<>(clazz, null, Jackson.newObjectMapper(),
      ...
      LoggerFactory.getLogger(JobConfiguration.class).error("Error reading job configuration: {}",
          e.getMessage(), e);
