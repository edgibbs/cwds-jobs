...
public abstract class DataAccessModule extends AbstractModule {
  ...
  @SuppressWarnings({"squid:S1147", "findbugs:DM_EXIT"}) // suppressing "Exit methods should not be
                                                         // called" since we really need it here
  public DataAccessModule(DataSourceFactory dataSourceFactory, String dataSourceName,
      List<Class<?>> entityClasses) {
    try {
      sessionFactory =
          SessionFactoryUtil.buildSessionFactory(dataSourceFactory, dataSourceName, entityClasses);
    } catch (Exception e) {
